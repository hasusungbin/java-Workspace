...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		// 이유는 얕은복사가 이루어졌기 때문 => 배열의 주소값이 복사되어
...
//		System.arraycopy(origin, 2, copy, 9, 2);
	...
	public void method4() {
		...
		int[] copy = Arrays.copyOf(origin, 10); // copy 배열의 크기 == 복사할 갯수
		 ...
		 * 갯수만큼 복사 진행됨. 내가 제시한 길이가 원본배열보다 크다면 나머지에는 0의값으로
		 * 채워서 복사해줌.
		 */
	}
	...
	public void method5() {
		...
		// 복사본 배열 = 원본배열.clone();
		int[] copy = origin.clone(); // 인덱스 직접 지정x , 복사할 갯수지정 x
		...
		/*
		 * Arrays.toString(배열)
		 * -> 앞과 뒤에 [] 각 값마다 ,를 넣어서 하나의 문자열로 연이어서 출력 해주는 메서드
		 */
