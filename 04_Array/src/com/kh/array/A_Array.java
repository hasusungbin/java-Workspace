...
public class A_Array { 
	...
	public void method1() {
		...
		// 각각의 인덱스값을 확인하기 위해서는 []블럭을 통해 내가 확인하고자 하는 인덱스(방번호)를 제시 해줘야함.
		// 그럼 arr 에 전체 담겨있는 값을 확인하고 싶다면?
		System.out.println(arr); // 실제 arr 이라는 값에는 주소값이 담겨있음.
	...
	public void method2() {
		...
		int i = 10; // 일반변수. i 라는 박스에 10이라는 값이 그냥 담겨있음.
		
		int[] arr = new int[5];
		// 배열 자료형
		/*
		 * 우선 대입연산자 기준 왼쪽이 먼저 실행되므로 arr 이라는 int형 배열 박스가 먼저 만들어짐.(stack영역에)
		 * 그 다음 int[5]짜리 방이 heap 영역에 생성됨.(new 키워드가 붙은녀석은 항상 heap 영역에 값이 할당됨)
	...
	public void method3() {
		 ...
		 * -> heap라는 영역에는 절~대로 빈공간이 존재할 수 없기때문.
...
//		arr[6] = 100;
		 ...
		 * => 배열의 크기를 변경 하고자 한다면 어쩔 수 없이 새로운 배열을 만들어야 함.
		 */
	...
	public void method8() {
		...
		//3. 반복문 활용해서 0번인덱스부터 ~ 마지막 인덱스까지 배열에 담긴값 출력.
		// 출력 예시) arr[x] : xx
	...
	public void method10() {
		...
		// 3. 문자열에서 각 인덱스 별로 문자를 뽑아서 char 배열의 각 인덱스에 담기
		for(int i = 0; i < arr.length; i++) {
			arr[i] = str.charAt(i);
		}
		// arr[0] = 문자열.charAt();
	...
	public void method11() {
		...
		// 3. 반복문 활용해서 0번인덱스부터~ 마지막 인덱스까지 배열에 담긴값 출력과 동시에,
	...
	public void method12() {
		...
		//1. 반복문을 활용해서 heights 배열의 0번인덱스부터 마지막 인덱스까지 반복을 진행하면서
