...
public class PolyRun {
	...
	public static void main(String[] args) {
		 ...
		 */
		System.out.println("1. 부모타입 래퍼런스변수로 부모객체를 다루는 경우");
		Parent p1 = new Parent();
		p1.printParent();
		// p1 래퍼런스변수로 Parent 에만 접근 가능.
		
		// 2. 자식타입 래퍼런스 변수로 자식 객체를 다루는 경우
		System.out.println("2. 자식타입 래퍼런스변수로 자식객체를 다루는 경우");
		...
		// c1 래퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 형변환된 채 진행됨.
		// 즉 자식에서 부모클래스 자동형변환(업캐스팅). -> 작은것에서 큰것으로 형변환 된것과 같은 이치.
		
		// 3. 부모타입 래퍼런스 변수로 자식 객체를 다루는 경우(다형성이 적용된 개념)
		System.out.println("3. 부모 타입 래퍼런스로 자식 객체를 다루는경우(다형성 적용된 개념)");
		...
		// 단, 다형성이 적용되면 부모타입 래퍼런스 변수로 모든 자식객체를 다 받아줄 수 있다.
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// instanceof(객체) 연산자 // 반환값 -> true, false
			// 현재 참조하고 있는 래퍼런스 변수가 실제로 어떤 클래스(객체)타입을 참조하고 있는지 확인할 때 사용.
...
//			}
			...
			arr[i].print();
			/*
			 *  * 동적바인딩 : 프로그램이 실행되기전에 자동으로 컴파일 되면서 정적으로 현재 레퍼런스 변수의 메소드를 가리키게됨.
