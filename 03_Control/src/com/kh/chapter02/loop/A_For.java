...
public class A_For {
	 ...
	 * 			(반복문에서 사용 할 변수를 선언 및 초기화 할 때 사용함.)
	 * 			ex)int i = 0;
	 * -조건식 : 반복문이 수행될 조건을 작성하는 구문
	 * 			조건식의 결과가 true 이면 반복문 실행
	 * 			조건식의 결과가 false 이면 반복문 멈추고, 빠져나옴
	 ...
	 * 		-> 조건식(조건검사) -> true일 경우 실행할 코드 실행 -> 증감식
	 * 		...
	 * 		-> 조건식(조건검사) -> false일 경우 실행할 코드 실행하지않음.
	 * 매번 조건식의 조건검사 결과값을 가지고 조건이 true일 경우만 반복해서 실행함.
	 * 
	 * 알아둘것 :
	 * for문안에 초기식, 조건식, 증감식 각각 생략이 가능하긴함. 단, 반드시 ; 은 작성해줘야함.
	 */
	public void method1() {
		//"안녕하세요"를 5번 반복해서 출력하고싶은 경우.
...
//		for(;;) { // 초기식 , 조건식 , 증감식이 없더라도 문법적 오류는 아님. 다만 무한 반복으로 실행된다.
//			System.out.print("야호");
...
//		}
	...
	public void method10() {
		// 사용자에게 문자열을 입력받아서 각 인덱스 별로 문자를 하나씩 뽑아서 출력
		// apple 길이 : 5
		// 01234
	...
	public void method12() {
		...
		//단수가 2단~ 9단 까지의 범위가 아니라면
		//잘못 입력했습니다.(2~9 사이의 정수 입력)
		
		//제대로 입력했다면 해당 단수를 출력
		Scanner sc = new Scanner(System.in);
		System.out.println("단을 입력하세요(2~9 사이의 정수 입력) :");
		int num = sc.nextInt();
		if(2 <= num && num <= 9) {
		for(int i = 1; i <= 9; i++) {
		System.out.printf("%d x %d = %d \n", num , i , (num*i));
			...
			System.out.println("잘못 입력했습니다.(2~9 사이의 정수 입력) : ");
		}
