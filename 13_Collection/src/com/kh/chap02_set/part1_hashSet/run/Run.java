package com.kh.chap02_set.part1_hashSet.run;

import java.util.ArrayList;
import java.util.HashSet;
import java.util.Iterator;

import com.kh.chap02_set.part1_hashSet.model.vo.Student;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println(hs1); // 중복 저장 x 저장 순서 유지 x [반갑습니다, 안녕하세요, 여러분]
		
		// 크기 구하기 : size() 		= hs1의 크기는 ? 3
		System.out.println("hs1의 크기는 ? "+hs1.size());
		
		// 값 삭제 : remove(삭제할 값)	= [반갑습니다, 안녕하세요]
		hs1.remove("여러분");
		...
		hs2.add(new Student("홍성빈", 27, 100));
		hs2.add(new Student("장상미", 23, 99));
		hs2.add(new Student("김귀요미", 23, 80));
		hs2.add(new Student("홍성빈", 27, 100));
		
		System.out.println(hs2);
		// 저장 순서 유지 x , 중복 저장 o -> 동일객체로 판단되지 않기 때문.
		// 값이 추가될 때 equals(), hashCode()로 비교 후 둘다 결과가 true면 동일객체로 판단.
		...
		// .iterator() : List계열과 Set계열에서만 사용 가능함. (Map계열에선 다른방법으로 사용가능)
	}
