...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 	프레임워크
		 * 효율적인 기능들이 이미 정의되었는 틀(프레임)
		 * 
		 * 	정리해보면
		 * 	데이터들이 새로이 추가되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘) 들이 이미 정의 되어 있는 틀이 있다.
		 * 	== 컬렉션
		 ...
		 *  	ex) HashSet, TreeSet
		 *  Map 계열 => 키(key) + 담고자 하는 값(value)세트로 저장 / 값 저장시 순서 유지 x(index 개념 없음)
		 *  	/ key 중복 허용 x
		 *  	ex) HashMap, Properties
		 */
		
		
		ArrayList<Music> list = new ArrayList<>(); // 내부적으로 크기 10짜리인 배열이 생성됨.
		...
		list.add(1 , new Music("메롱","홍성빈"));
		// 중간에 값 추가 시 알아서 기존의 값들을 뒤로 땡겨주는 작업이 내부적으로 진행됨.
		
		System.out.println(list);
		
		// 3. set(int index , E e) : 리시트에 원하는 위치의 인덱스값을 제시하면 전달하는 e값으로 변경해주는 메소드
		list.set(0, new Music("바보","장상미"));
		
		System.out.println(list);
		
		// 4. remove(int index) : 리스트에서 해당 인덱스 위치에 있는 값을 삭제 시켜주는 메소드.
		list.remove(1);
		...
		// 6. get(int index) : E ==> 리스트에 해당 index위치의 데이터를 반환 해주는 메소드.
		// 제네릭 설정 전
