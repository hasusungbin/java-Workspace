...
//			System.out.println("0으로 나눌 수 없습니다.");
	...
	public void method2() {
		...
		System.out.print("정수 입력(0 제외) : "); // InputMismatchException : 정수 이외의 값을 입력시 발생함. 
		
		try {
			int num = sc.nextInt();
			
			System.out.println("나눗셈 연산 결과 : " +(10 / num)); // 예외 발생할 수 있음.
		} catch(ArithmeticException e) {
			System.out.println("0으로 나눌 수 없습니다.");
	...
	public void method3() {
		...
		// ArrayIndexOutOfBoundsException : 배열의 부적적한 인덱스로 접근시 발생하는 예외
		// NegativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
...
//		int[] arr = new int[Math.abs(size)]; // 방법2. 항상 양의 정수값이 들어갈 수 있도록 abs메소드 사용.
//		}
...
//		} 
		...
		} // 다중 catch 에서는 블럭의 순서가 중요함. => 범위가 작은 자식타입의 예외클래스를 먼저 기술해야함.
	 ...
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생했을 때를 대비해서 그 때 실행할 내용을 정의해 두는것.
