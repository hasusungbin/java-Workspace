...
public class Server {
	 ...
	 * 3) 요청이 오면 수락
	 * 4) 클라이언트 정보를 저장
	 * - Socket client = new server.accept();
	...
	public void serverStart() {
		...
		try {
			...
			while(true) {
				// 3) 접속 요청이 오면 요청 수락(accept())후 해당 클라이언트에 대한 소켓객체 생성
				// 4) 클라이언트의 정보를 저장.
				...
				PrintWriter pw = new PrintWriter(os, true); // PrintWriter -> 데이터를 출력할때 print(),
				// println() 메소드를 사용
				
				// 7) 스트림을 통해 읽고 쓰기.
				while(true) {
					String message = br.readLine(); // 클라이언트로 부터 입력이 있기전까지 멈춤.(블로킹)
