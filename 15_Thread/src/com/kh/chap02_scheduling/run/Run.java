...
public class Run {
	...
	public static void main(String[] args) {
		...
		// setPriority() : 우선순위를 지정해 주는 메소드 (직접 보기는 힘듦)
		...
		/*
		 * 메인 스레드가 종료 되더라도 실행 중인 스레드가 하나라도 있다면 프로세스는 종료되지 않는다.
		 * 해결책 : start()시키기전에 우선순위, 주종관계를 설정해야함.
		 * 
		 * 주종관계 설정
		 * main스레드가 종료시 다른스레드도 종료시키기 위해서 반드시 setDaemon사용.
		 * 주의점은 스레드중 한개라도 setDaemon으로 설정 시 모든 스레드에 steDaemon설정을 해줘야함.
		 */
		System.out.println("메인 스레드의 끝 부분");
		// 시작순서는 우선순위와 다를 수 있지만 종료순서는 우선순위(priority)가 높은순으로 끝나게됨.(항상 그렇지 않음.)
	}
